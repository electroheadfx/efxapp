  ...
  $.FE.PLUGINS.image = function (editor) {
      ...
      if ($handler && $current_image) {
        ...
        if (editor.opts.imageResizeWithPercent) {
          ...
          if (width + diff_x >= editor.opts.imageMinWidth) {
            ...
            if (hasCaption()) {
              $current_image.parent().css('width', width + diff_x);
            }
    ...
    function insertByURL () {
      ...
      if ($input.val().length > 0) {
        ...
        if (editor.opts.imageUploadRemoteUrls && editor.opts.imageCORSProxy) {
          var xhr = new XMLHttpRequest();
          xhr.open('GET', editor.opts.imageCORSProxy + '/' + $input.val(), true);
    ...
    function _init () {
      ...
      editor.events.on('window.cut window.copy', function (e) {
        ...
        if ($current_image && editor.popups.isVisible('image.edit') && !editor.popups.get('image.edit').find(':focus').length) {
          ...
          editor.paste.saveCopiedText(e, $current_image.get(0).outerHTML, $current_image.attr('alt'));
    ...
    function _setStyle ($img, _display, _align) {
      ...
      // Do not add style when it is not necessary
      if (editor.opts.imageEditButtons.indexOf('imageAlign') < 0 && editor.opts.imageEditButtons.indexOf('imageDisplay') < 0) {
        return false;
      }
    ...
    function toggleCaption () {
      ...
      if ($current_image && !hasCaption()) {
        ...
        $el.wrap('<span contenteditable="false" class="fr-img-caption ' + $current_image.attr('class') + '" draggable="false"></span>');
        ...
        $current_image.removeAttr('class');
        $current_image.parent().css('width', $current_image.width());
        ...
        $current_image.attr('class', $el.attr('class').replace('fr-img-caption', ''));
        $el.remove();
